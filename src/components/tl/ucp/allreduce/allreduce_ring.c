...
void ucc_tl_ucp_allreduce_ring_progress(ucc_coll_task_t *coll_task)
{
    ...
    while (task->tagged.send_posted < tsize - 1) {
        ...
        rblock = task->allreduce_ring.get_recv_block(&task->subset, trank,
                                                     tsize, step);
        // TODO: Reduce on previous blocks?-------------------------------
        // TODO: What is the meaning of reduction?
        ...
        if (UCC_OK !=
            (status = ucc_dt_reduce(
                 ...
                 AVG_ALPHA(task), task->allreduce_ring.executor,
                 &task->allreduce_ring.etask))) {
            tl_error(UCC_TASK_LIB(task), "failed to perform dt reduction");
            task->super.status = status;
            return;
        }
        EXEC_TASK_WAIT(task->allreduce_ring.etask);
        //----------------------------------------------------------------
